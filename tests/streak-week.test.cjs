...
assert.match(styles,/\.streak-week-day\.today\{position:relative;z-index:1;[^}]*transform:scale\(1\.08\);transform-origin:center center\}/);
...
assert.match(source,/const animate=day\.today&&animateToday/);
assert.match(source,/<strong class="\$\{animate\?'streak-fire-today':''\}"[^>]*><i class="\$\{animate\?'streak-fire-flicker':''\}"/);
assert.doesNotMatch(styles,/\.streak-fire-today\{[^}]*font-size/);
assert.match(styles,/\.streak-fire-flicker\{animation:streak-fire-flicker 1\.1s ease-in-out infinite;transform-origin:bottom center\}/);

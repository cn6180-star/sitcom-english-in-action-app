...
assert.equal(dialogues.length,326);assert.equal(rows.length,2201);
assert.deepEqual({auto,explicit,excluded},{auto:1948,explicit:247,excluded:6});
...
assert.equal(nonS1.length,1324);assert.equal(hash(nonS1),'7ae52a48f0dacc921b6fc331317413e60ef5df24d58bc7738a5940019659aaf8','non-S1 matcher ranges match the reviewed Notion Dialogue synchronization and S3 cleanup implementation');
...
assert.equal(Object.keys(hints).length,279);assert.equal(Object.values(hints).filter(hint=>hint.overrideMatcher).length,179);
...
console.log(`Highlight coverage: ${auto} auto, ${explicit} explicit, ${excluded} approved mismatches; 2,201 links, offsets, overlap and non-S1 integrity PASS`);

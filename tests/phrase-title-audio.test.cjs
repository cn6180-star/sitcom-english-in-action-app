...
assert.match(source,/<div class="phrase-title-row"><h1 class="detail-title">\$\{esc\(p\.phrase\)\}<\/h1><button class="speaker-button phrase-title-speaker"[^>]+data-text="\$\{esc\(p\.phrase\)\}"[^>]+onclick="speakExample\(this\.dataset\.text\)"[^>]+aria-label="Read phrase aloud">🔊<\/button><\/div>/);
...
for(const name of ['speechAvailable','updateDialoguePlaybackButton','clearDialogueVoiceWait','stopDialoguePlayback','speakExample']){
  const line=source.split(/\r?\n/).find(value=>value.startsWith(`function ${name}(`));assert.ok(line,name);vm.runInContext(line,context);
}
context.speakExample('if I had a nickel for ~');
context.speakExample('If I had a nickel for every mistake, I’d be rich.');
assert.deepEqual(context.window.speechSynthesis.spoken,['if I had a nickel for ~','If I had a nickel for every mistake, I’d be rich.']);
assert.equal(context.window.speechSynthesis.cancelCalls,2,'each new phrase/example playback cancels prior speech');
assert.equal(context.dialoguePlaybackRun,2,'phrase playback follows existing dialogue interruption behavior');

console.log('Phrase title audio: shared TTS, cancellation, Example isolation and responsive title layout PASS');

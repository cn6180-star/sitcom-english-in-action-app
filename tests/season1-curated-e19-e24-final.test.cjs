...
assert.equal(phrases.length,3924);assert.equal(byId.size,3924);assert.equal(Math.max(...phrases.map(p=>+p.id.slice(1))),4051);assert.equal(phrases.filter(p=>p.episode.startsWith("S01")).length,1623);
...
assert.equal(ordered.length,3781);assert.equal(hash(ordered.map(p=>[p.id,p.episode,p.sourceOrder])),"36b6d5020278962ca397480a7fcddb35476170f2e6ed4e0a287ff9546c8b68da");
...
assert.equal(dialogues.length,326);assert.equal(hash(dialogues.filter(d=>Number(d.id.slice(1))<=167)),"8c3289733bdf9cef958b7af354fc74dfffb35629edc6e6d8272f992b8349c2d2");assert.deepEqual(dialogues.flatMap(d=>(d.phraseLinks||[]).filter(id=>!byId.has(id))),[]);

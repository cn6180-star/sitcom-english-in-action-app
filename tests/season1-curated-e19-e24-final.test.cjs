...
assert.equal(phrases.length,3707);assert.equal(byId.size,3707);assert.equal(Math.max(...phrases.map(p=>+p.id.slice(1))),3781);assert.equal(phrases.filter(p=>p.episode.startsWith("S01")).length,1623);
...
assert.equal(ordered.length,3212);assert.equal(hash(ordered.map(p=>[p.id,p.episode,p.sourceOrder])),"29f158f70f81ecd6723cf5b6d9a2daf9edd7a318d4021241230862e8df4ba55a");
...
assert.equal(dialogues.length,326);assert.equal(hash(dialogues.filter(d=>Number(d.id.slice(1))<=167)),"c63db921cc29711193bd54d17ca2720312551e3957249fba2e682fa637f1bbfc");assert.deepEqual(dialogues.flatMap(d=>(d.phraseLinks||[]).filter(id=>!byId.has(id))),[]);

...
assert.equal(phrases.length,3946);assert.equal(byId.size,3946);assert.equal(Math.max(...phrases.map(p=>+p.id.slice(1))),4116);assert.equal(phrases.filter(p=>p.episode.startsWith("S01")).length,1623);
...
assert.equal(ordered.length,3902);assert.equal(hash(ordered.map(p=>[p.id,p.episode,p.sourceOrder])),"bc1f73487235670e8837d0dd40e75d28db38261f46c4276c5ca6fc406388b53c");
...
assert.equal(dialogues.length,326);assert.equal(hash(dialogues.filter(d=>Number(d.id.slice(1))<=167)),"ea4c7c2f3c3528ea58f87c03b86c712860a9cfad0ff6e2564dd2468dd22b5b1d");assert.deepEqual(dialogues.flatMap(d=>(d.phraseLinks||[]).filter(id=>!byId.has(id))),[]);

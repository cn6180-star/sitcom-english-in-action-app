...
assert.equal(phrases.length,3669);assert.equal(byId.size,3669);assert.equal(Math.max(...phrases.map(p=>+p.id.slice(1))),3739);assert.equal(phrases.filter(p=>p.episode.startsWith("S01")).length,1623);
...
assert.equal(ordered.length,3132);assert.equal(hash(ordered.map(p=>[p.id,p.episode,p.sourceOrder])),"4aa268f3060758b9c758af2c366b58cdeda3e26e74d5550cdb5e197a24696fda");
...
assert.equal(dialogues.length,326);assert.equal(hash(dialogues.filter(d=>Number(d.id.slice(1))<=167)),"b47732f453ef75aa4e3116ca23cd2b879f9606e3743ac63eb1a5e10ef5695034");assert.deepEqual(dialogues.flatMap(d=>(d.phraseLinks||[]).filter(id=>!byId.has(id))),[]);

...
assert.equal(phrases.length,3977);assert.equal(byId.size,3977);assert.equal(Math.max(...phrases.map(p=>+p.id.slice(1))),4116);assert.equal(phrases.filter(p=>p.episode.startsWith("S01")).length,1623);
...
assert.equal(ordered.length,3933);assert.equal(hash(ordered.map(p=>[p.id,p.episode,p.sourceOrder])),"6c17d6f0fcd038b29d54ebb03ec4de5bb149366f62c5d1febfec2fba3f0e2cc3");
...
assert.equal(dialogues.length,326);assert.equal(hash(dialogues.filter(d=>Number(d.id.slice(1))<=167)),"5a5d2fa0928f2044f64cefe417b8ca32a43a985561e67d2ba5e5d0141d0f040a");assert.deepEqual(dialogues.flatMap(d=>(d.phraseLinks||[]).filter(id=>!byId.has(id))),[]);

...
assert.equal(phrases.length,3744);assert.equal(byId.size,3744);assert.equal(Math.max(...phrases.map(p=>+p.id.slice(1))),3821);assert.equal(phrases.filter(p=>p.episode.startsWith("S01")).length,1623);
...
assert.equal(ordered.length,3287);assert.equal(hash(ordered.map(p=>[p.id,p.episode,p.sourceOrder])),"39e6719f4d06fadb4e380e39aab9bb45d33c579db6c93e832f64f67817d44fc3");
...
assert.equal(dialogues.length,326);assert.equal(hash(dialogues.filter(d=>Number(d.id.slice(1))<=167)),"f74d150a08f29c6d019f87ad7e6ce053dd17f692d907d304ca2c18f69147ac45");assert.deepEqual(dialogues.flatMap(d=>(d.phraseLinks||[]).filter(id=>!byId.has(id))),[]);

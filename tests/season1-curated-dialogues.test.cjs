...
assert.equal(dialogues.length,200);assert.equal(sets[0].dialogues.length,43);
assert.equal(new Set(dialogues.map(dialogue=>dialogue.id)).size,200);assert.equal(phraseIds.size,3106);

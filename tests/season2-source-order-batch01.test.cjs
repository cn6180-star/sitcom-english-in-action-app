...
assert.equal(phrases.length,3547);assert.equal(new Set(phrases.map(p=>p.id)).size,3547);
...
assert.equal(hash(restored),'4b07536e1421a2f5a9068dde04106ab862d1bd295fcf747e21c53fc77c0a1728');
...
assert.equal(hash([...membership].sort((a,b)=>Number(a[0].slice(1))-Number(b[0].slice(1)))),'d2edb6367ff944c3a36f1f400c697fd22d9de17b7fdfcb1cc0622d09dfd869d1');
const layout=datasets.flatMap(x=>x.data.phrases.map((p,i)=>[x.file,i,verified.has(p.id)?'TARGET:'+p.episode:p.id]));
assert.equal(hash(layout),'dc4b81784a82a5777d476ef6ff1c9dcc47c88b35605728eea3acaa99a11be8c1');
...
assert.equal(hash(dialogues),'ca07ef8069a3146ae354fd9a93ab20770721481ffdbbbfe5dc3bf423473a595b');
assert.equal(hash(dialogues.filter(d=>Number(d.id.slice(1))<=167)),'088a05d1d96b275f1bb5e8f8031fdc8b52a3da6e4f5c288981be75edc691078a');

...
assert.equal(phrases.length,3810);assert.equal(new Set(phrases.map(p=>p.id)).size,3810);
assert.equal(Math.max(...phrases.map(p=>Number(p.id.slice(1)))),3908);
...
assert.equal(hash(restored),'fca8d8f2f68830f760f6a9c43f6cff6970ca2689205a0e747c63181a715e6520');
...
assert.equal(hash([...membership].sort((a,b)=>Number(a[0].slice(1))-Number(b[0].slice(1)))),'4cc1238dd339b0a793dc347f1bbfe2f74ecd651e11112a6ffd0a402bb982f759');
const layout=datasets.flatMap(x=>x.data.phrases.map((p,i)=>[x.file,i,verified.has(p.id)?'TARGET:'+p.episode:p.id]));
assert.equal(hash(layout),'0535877ccfebf9698abd6cbe19e3e655881c6494a7d89313d5c6381a868a6542');
...
assert.equal(hash(dialogues.filter(d=>Number(d.id.slice(1))<=167)),'71834ae359e4d65e3f13d1d07918c1e1660ae03e0762281a4e3330cee7149ddf');

...
assert.equal(phrases.length,3946);assert.equal(new Set(phrases.map(p=>p.id)).size,3946);
...
assert.equal(hash(restored),'d9fc07f3eb376e6453d7fef921f8da44ef48af5838e2d72a23bbccae637b80c4');
...
assert.equal(hash([...membership].sort((a,b)=>Number(a[0].slice(1))-Number(b[0].slice(1)))),'77cf46915a8c20d62bbdfe7f18984378a6b371564514b2a30734cb798c371541');
const layout=datasets.flatMap(x=>x.data.phrases.map((p,i)=>[x.file,i,verified.has(p.id)?'TARGET:'+p.episode:p.id]));
assert.equal(hash(layout),'ce9c2f2bef65e8150ed833343e17d1ecbcb7697476b78fa0c55c850a30694a9f');

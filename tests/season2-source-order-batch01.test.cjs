...
assert.equal(phrases.length,3866);assert.equal(new Set(phrases.map(p=>p.id)).size,3866);
assert.equal(Math.max(...phrases.map(p=>Number(p.id.slice(1)))),3988);
...
assert.equal(hash(restored),'cad4f2418ee84e9316705503199419290ddcf749c604bd90766cc7e2d008a0e7');
...
assert.equal(hash([...membership].sort((a,b)=>Number(a[0].slice(1))-Number(b[0].slice(1)))),'dd68791ce8c4e102222e8ffe2d7bbc256afb7651e3876274abd7b31a580e4076');
const layout=datasets.flatMap(x=>x.data.phrases.map((p,i)=>[x.file,i,verified.has(p.id)?'TARGET:'+p.episode:p.id]));
assert.equal(hash(layout),'8a601f0a95d578640faedb3a9245f084aebf5871998f460442f63dea061bbf94');
...
assert.equal(hash(dialogues.filter(d=>Number(d.id.slice(1))<=167)),'d14c44b70222e6c2880f556f04d568cc8063eaa99cace6f832420f71aac141d6');

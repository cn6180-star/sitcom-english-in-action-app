...
assert.equal(phrases.length,3744);assert.equal(new Set(phrases.map(p=>p.id)).size,3744);
...
assert.equal(hash(restored),'682f9ee54d0bbfb471e40a9e0169a3fb431a5c82a47f734bf4bbd15b5b47f302');
...
assert.equal(hash([...membership].sort((a,b)=>Number(a[0].slice(1))-Number(b[0].slice(1)))),'7f95c28c6f9982d9cc6e7e8a13f3bfd2c5acfc0118a91dd7b75b71450d9db145');
const layout=datasets.flatMap(x=>x.data.phrases.map((p,i)=>[x.file,i,verified.has(p.id)?'TARGET:'+p.episode:p.id]));
assert.equal(hash(layout),'356dbcc757e406409852a2d61258f67bfb8d530cc05c3d37878ed1c0271121e5');

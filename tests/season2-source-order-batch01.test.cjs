...
assert.equal(phrases.length,3924);assert.equal(new Set(phrases.map(p=>p.id)).size,3924);
...
assert.equal(hash(restored),'06701214f11bdd1278699b42deb0c2408cf7978209ea3e1a4024f4d4aa029e9f');
...
assert.equal(hash([...membership].sort((a,b)=>Number(a[0].slice(1))-Number(b[0].slice(1)))),'03c46d68b79585e05ab6749cded3d8997c1de98f8a6c266bf39418fe2483228e');
const layout=datasets.flatMap(x=>x.data.phrases.map((p,i)=>[x.file,i,verified.has(p.id)?'TARGET:'+p.episode:p.id]));
assert.equal(hash(layout),'b88abe576aed910b82de67780fe48167178ac16d3b6e191a78603ab9bb1cdf83');

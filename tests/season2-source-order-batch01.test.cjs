...
assert.equal(phrases.length,3707);assert.equal(new Set(phrases.map(p=>p.id)).size,3707);
...
assert.equal(hash(restored),'c67f64904fb02158351e559ca4fad5bdbfa80f339f46390533d3f481261f4f7d');
...
assert.equal(hash([...membership].sort((a,b)=>Number(a[0].slice(1))-Number(b[0].slice(1)))),'36d002748be05c097b5e709bc1c66aa8a4f4ecb92a823a69132f1cc832e1c881');
const layout=datasets.flatMap(x=>x.data.phrases.map((p,i)=>[x.file,i,verified.has(p.id)?'TARGET:'+p.episode:p.id]));
assert.equal(hash(layout),'21add30f48a0b903e02ed6ff3a975eb8bfe458b98b8f2db008f9793b35437e48');

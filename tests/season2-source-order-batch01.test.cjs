...
assert.equal(phrases.length,3591);assert.equal(new Set(phrases.map(p=>p.id)).size,3591);
...
assert.equal(hash(restored),'15bdaf8f11ac2cf691c985d046390c9d8131fa2b589f52fa03aecd128f8da986');
...
assert.equal(hash([...membership].sort((a,b)=>Number(a[0].slice(1))-Number(b[0].slice(1)))),'384520fd48ed17eb14bd52113e20c92e102920289054a53d531781eade0a7578');
const layout=datasets.flatMap(x=>x.data.phrases.map((p,i)=>[x.file,i,verified.has(p.id)?'TARGET:'+p.episode:p.id]));
assert.equal(hash(layout),'d5354e8cefe08f95c965623ff49881f3e7990ab4c1fe24b0f642f7425bc31c5d');

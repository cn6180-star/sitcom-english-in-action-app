...
assert.equal(phrases.length,3977);assert.equal(new Set(phrases.map(p=>p.id)).size,3977);
assert.equal(Math.max(...phrases.map(p=>Number(p.id.slice(1)))),4116);
...
assert.equal(hash(restored),'162aa82400a5992feaa4f78397fc8136ab1982a9b5bae3295a6d0539caa03129');
...
assert.equal(hash([...membership].sort((a,b)=>Number(a[0].slice(1))-Number(b[0].slice(1)))),'5cca3dac449eb273fadf0b495e0ffb229490672585345a1c1c8faf5ac36f3606');
const layout=datasets.flatMap(x=>x.data.phrases.map((p,i)=>[x.file,i,verified.has(p.id)?'TARGET:'+p.episode:p.id]));
assert.equal(hash(layout),'2c079208a3f5abc87be8838191ba2ef07110ce3f172e3256c4e7a859d31edfe4');
...
assert.equal(hash(dialogues),'b7db948af4a4c067eaff5eed344daf1941deaa6e54cd9993529331656c7b83b7');
assert.equal(hash(dialogues.filter(d=>Number(d.id.slice(1))<=167)),'5a5d2fa0928f2044f64cefe417b8ca32a43a985561e67d2ba5e5d0141d0f040a');

...
let rows=[],highlights=[],actions=null,phraseButton=null,rendered=0;
...
const controls={insertAdjacentElement:(_,el)=>{actions=el}};
c.app={querySelector:s=>s==='.blank-mode-actions'?actions:s==='.blank-mode-hint'?hint:s==='.dialogue-side'?side:s==='.dialogue-controls'?controls:null,querySelectorAll:s=>s==='.bubble-row'?rows:s==='[data-blank-key]'?highlights.filter(h=>h.dataset.blankKey):s==='.dialogue-controls .seg-button'?buttons:[]};
...
const names=['setPractice','revealDialogueBlank','setDialogueBlanksRevealed','toggleDialogueBlankPhrases','updateDialogueBlankPhraseList','dialogueMove','dialoguePracticeDelay','englishDialogueVoices','playDialogue'];
...
c.revealDialogueBlank('p1');assert.equal(visible(d,0),d.lines[0][1]);c.setDialogueBlanksRevealed(false);assert.equal(visible(d,0),'I’m _____.');
d=sample('It’s hard to keep cravings at bay.',['keep','at bay']);c.setPractice('blank');assert.equal(visible(d,0),'It’s hard to _____ cravings _____.');highlights[0].onkeydown({key:'Enter',preventDefault(){}});assert.equal(visible(d,0),d.lines[0][1],'phrase-wide multi-range reveal');c.setDialogueBlanksRevealed(false);assert.equal(highlights.filter(h=>h.classes.has('dialogue-blank')).length,2);c.setDialogueBlanksRevealed(true);assert.equal(visible(d,0),d.lines[0][1]);
c.toggleDialogueBlankPhrases();assert.equal(list.hidden,false);c.setPractice('normal');assert.equal(visible(d,0),d.lines[0][1]);assert.equal(actions.hidden,true);assert.equal(list.hidden,false);
...
let checked=0;
for(const real of dialogues){const matches=c.realMatches(real,real.phraseLinks.map(id=>phrases.find(p=>p.id===id)));mount(real,matches);c.setPractice('blank');rows.forEach((row,i)=>row.selected.forEach((m,j)=>{assert.equal(row.spans[j].textContent,real.lines[i][1].slice(m.index,m.index+m.length));assert.equal(row.spans[j].dataset.blankKey,m.phraseId);assert.ok(row.spans[j].classes.has('dialogue-blank'));checked++}));c.setDialogueBlanksRevealed(true);real.lines.forEach((line,i)=>assert.equal(visible(real,i),line[1]));c.setDialogueBlanksRevealed(false);assert.ok(highlights.every(h=>h.classes.has('dialogue-blank')));c.setPractice('normal');real.lines.forEach((line,i)=>assert.equal(visible(real,i),line[1]));}
assert.ok(checked>1000);
for(const [id,pid,target] of [['d44','p408','call it first'],['d204',null,'What if']]){const real=dialogues.find(d=>d.id===id),matches=c.realMatches(real,real.phraseLinks.map(id=>phrases.find(p=>p.id===id)));mount(real,matches);c.setPractice('blank');assert.ok(highlights.some(h=>h.textContent===target&&h.classes.has('dialogue-blank')&&(!pid||h.dataset.blankKey===pid)));}
const excluded=dialogues.find(d=>d.id==='d8');mount(excluded,c.realMatches(excluded,[phrases.find(p=>p.id==='p117')]));c.setPractice('blank');assert.equal(highlights.length,0);excluded.lines.forEach((l,i)=>assert.equal(visible(excluded,i),l[1]));
// Original English, not rendered DOM, is the TTS source even in Blank mode.
const speech=[];c.window={speechSynthesis:{getVoices:()=>[{lang:'en-US'}],speak:u=>{speech.push(u.text);u.onend()},addEventListener:()=>{}}};c.document.hidden=false;c.SpeechSynthesisUtterance=function(text){this.text=text};c.stopDialoguePlayback=()=>{};c.speechAvailable=()=>true;c.updateDialoguePlaybackButton=()=>{};c.clearDialogueVoiceWait=()=>{};c.dialoguePlaybackRun=1;c.dialoguePlaybackPauseTimer=null;c.clearTimeout=()=>{};c.setTimeout=f=>{f();return 1};c.playDialogue();assert.deepEqual(speech,excluded.lines.map(l=>l[1]));

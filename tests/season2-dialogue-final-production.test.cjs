...
assert.equal(phrases.length,3624);assert.equal(phraseById.size,3624);
...
assert.equal(rows.length,2227);assert.deepEqual({fullAuto,fullExplicit,excluded},{fullAuto:1973,fullExplicit:248,excluded:6});
...
console.log('Dialogue production continuity: 326 total / S1 108 / S2 81; existing S2 survivors retained; all 2,227 links covered, 6 approved exclusions; Phrase data unchanged PASS');

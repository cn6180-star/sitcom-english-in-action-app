...
assert.equal(phrases.length,3866);assert.equal(phraseById.size,3866);
...
assert.equal(rows.length,2176);assert.deepEqual({fullAuto,fullExplicit,excluded},{fullAuto:1926,fullExplicit:244,excluded:6});
...
console.log('Dialogue production continuity: 326 total / S1 108 / S2 81; existing S2 survivors retained; all 2,176 links covered, 6 approved exclusions; Phrase snapshots PASS');

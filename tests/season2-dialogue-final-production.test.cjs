...
assert.equal(phrases.length,3744);assert.equal(phraseById.size,3744);
...
assert.equal(rows.length,2216);assert.deepEqual({fullAuto,fullExplicit,excluded},{fullAuto:1962,fullExplicit:248,excluded:6});
...
console.log('Dialogue production continuity: 326 total / S1 108 / S2 81; existing S2 survivors retained; all 2,216 links covered, 6 approved exclusions; Phrase snapshots PASS');

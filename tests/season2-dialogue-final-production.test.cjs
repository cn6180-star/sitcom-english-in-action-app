...
assert.equal(phrases.length,3669);assert.equal(phraseById.size,3669);
...
assert.equal(rows.length,2223);assert.deepEqual({fullAuto,fullExplicit,excluded},{fullAuto:1969,fullExplicit:248,excluded:6});
...
console.log('Dialogue production continuity: 326 total / S1 108 / S2 81; existing S2 survivors retained; all 2,223 links covered, 6 approved exclusions; Phrase data unchanged PASS');

...
assert.equal(phrases.length,3484);assert.equal(phraseById.size,3484);
...
assert.equal(rows.length,2232);assert.deepEqual({fullAuto,fullExplicit,excluded},{fullAuto:1977,fullExplicit:249,excluded:6});
...
console.log('Dialogue production continuity: 326 total / S1 108 / S2 81; existing S2 survivors retained; all 2,233 links covered, 6 approved exclusions; Phrase data unchanged PASS');

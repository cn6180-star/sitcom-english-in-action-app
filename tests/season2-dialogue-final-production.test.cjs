...
assert.equal(phrases.length,3810);assert.equal(phraseById.size,3810);
...
assert.equal(rows.length,2201);assert.deepEqual({fullAuto,fullExplicit,excluded},{fullAuto:1948,fullExplicit:247,excluded:6});
...
console.log('Dialogue production continuity: 326 total / S1 108 / S2 81; existing S2 survivors retained; all 2,201 links covered, 6 approved exclusions; Phrase snapshots PASS');

...
assert.equal(phrases.length,3977);assert.equal(phraseById.size,3977);
...
assert.equal(rows.length,2163);assert.deepEqual({fullAuto,fullExplicit,excluded},{fullAuto:1915,fullExplicit:242,excluded:6});
...
console.log('Dialogue production continuity: 326 total / S1 108 / S2 81; existing S2 survivors retained; all 2,163 links covered, 6 approved exclusions; Phrase snapshots PASS');
